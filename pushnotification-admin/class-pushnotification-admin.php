...
class Push_Notification_List_Table extends WP_List_Table {
	...
	function process_bulk_action() {
		...
		if( 'delete'===$this->current_action() ) {			
			...
			foreach($array_data as $key => $id_value){ 				
                $wpdb->query( "DELETE FROM $all_pushnotification_logs WHERE log_id = ".$id_value);				
			}			
	...
	function prepare_items() {
		...
		$data = $wpdb->get_results("SELECT * FROM $all_pushnotification_logs $where ORDER BY $orderby $order LIMIT $per_page OFFSET $paged", ARRAY_A);		
		$columns = $this->get_columns();
		$hidden = array();
		$sortable = $this->get_sortable_columns();
		$this->_column_headers = array($columns, $hidden, $sortable);
		$current_page = $this->get_pagenum();		
		$total_items = $wpdb->get_var("SELECT COUNT(log_id) FROM $all_pushnotification_logs");		
		$this->items = $data;		
